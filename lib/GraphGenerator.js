...
define(['RaphaelCanvas', 'VertexFactory', 'EdgeFactory', 'Utilities'], function(raphaelCanvas, vertexFactory, edgeFactory, utilities) {
    ...
    function createNewGraph(difficultyLevel) {
        vertexArray.forEach(function(element, index, array){element.remove();});
        delete vertexArray;
        vertexArray = [];
        edgeArray.forEach(function(element, index, array){element.remove();});
        delete edgeArray;
        edgeArray = [];
    ...
    function checkEdgeIntersection() {
        ...
        while (1 < edges.length) {
            ...
            var currentEdgeIntersects = false;
            edges.forEach(function(edge) {
                if (edge.intersectWith(currentEdge)) {
                    edge.markIntersecting();
                    currentEdgeIntersects = true;
                    ++intersections;
                    return true;
                } else {
                    return false;
            ...
            if (currentEdgeIntersects)
                currentEdge.markIntersecting();
        }
