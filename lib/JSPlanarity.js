 ...
 */
define(['RaphaelCanvas', 'VertexFactory', 'Utilities', 'Options'], function(raphaelCanvas, vertexFactory, utilities, options) {
window.JSPlanarity = (function(raphaelCanvas, utilities, options) {
    /**
     ...
     */
    function createFactories(raphaelCanvas) {
        /**
         * Create an Edge.
         */
        function createEdgeBetween(vertex1, vertex2) {
            ...
            var edgeVisualisation = raphaelCanvas.path().attr({
                'stroke-width' : options.edgeStrokeWidth
            });
            ...
            function activate() {
                edgeVisualisation.attr({
                    stroke : options.activeEdgeColor
                });
            ...
            function deactivate() {
                edgeVisualisation.attr({
                    stroke : options.inactiveEdgeColor
                });
         ...
         */
        return {createEdgeBetween : createEdgeBetween};
    }
     ...
     */
    function init() {
        factories = createFactories(raphaelCanvas);

        createNewGraph(options.difficulty);
    }
    ...
    function generatePlanarGraph(numberOfLines) {
        ...
        for ( var lineIndex = 0; lineIndex < lineRepresentations.length; lineIndex++) {
            ...
            for ( var edgeIndex = 0; edgeIndex < M.length - 1; edgeIndex++) {
                ...
                edgeArray.push(factories.createEdgeBetween(vertexArray[u], vertexArray[v]));
            }
        }
    ...
    function arrangeVerticesInCircle() {
        ...
        var hypothenuse = Math.min(options.xSize, options.ySize) / 2 - 10;
        ...
        for ( var vertexIndex = 0; vertexIndex < arrayLength; vertexIndex++) {
            ...
            var newX = Math.cos(currentAngle) * hypothenuse + options.xSize / 2;
            var newY = Math.sin(currentAngle) * hypothenuse + options.ySize / 2;

            currentVertex.setPosition(newX, newY, options.startupAnimationTimeMs);
        }
    }
    ...
    function checkEdgeIntersection() {
        ...
        if (0 < intersections.length)
            alert('There are ' + intersections.length + ' intersections left to clean up.');
        else {
...
}(raphaelCanvas, utilities, options));
});
