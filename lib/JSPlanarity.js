...
define(['icons', 'RaphaelCanvas', 'GraphGenerator', 'Options'], function(icons, raphaelCanvas, graphGenerator, options) {
    window.JSPlanarity = (function(RaphaelCanvas, graphGenerator, options) {
        ...
        var checkButton = RaphaelCanvas.path(icons["checkcircle"]).attr({fill: "#333", stroke: "none"});
        checkButton.translate(options.getItem("xSize") - 40, options.getItem("ySize") - 160);
        checkButton.click(function(){
            checkSolution();
        });
        
        var resetButton = RaphaelCanvas.path(icons["crosscircle"]).attr({fill: "#333", stroke: "none"});
        resetButton.translate(options.getItem("xSize") - 40, options.getItem("ySize") - 120);
        resetButton.click(function(){
            generate();
        });
        
        var selectButton = RaphaelCanvas.path(icons["arrowalt"]).attr({fill: "#333", stroke: "none"});
        selectButton.translate(options.getItem("xSize") - 40, options.getItem("ySize") - 80);
        ...
        infoButton.click(function(){
            alert("This is JSPlanarity\n\nLicensed under GPLv3+\n\nWritten by Manfred Kroehnert\n\nhttp://mkroehnert.github.com/JSPlanarity");
        });
         ...
         */
         ...
         function selectLevel() {
            var difficulty = parseInt(window.prompt('Enter Level Number', '1'), 10);
            if (difficulty < 1)
                difficulty = 1;
